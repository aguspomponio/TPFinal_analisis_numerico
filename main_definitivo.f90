...
deltat = 0.002
angulo = 20
alfa = (angulo*2*pi)/360.

!Componentes de ambas velocidades iniciales
...
OPEN(UNIT=2, FILE='Trayectoria_ES.dat', STATUS='REPLACE')
...
paso = cambiopaso(v,paso,tolerancia,roce,masa,1,hmax,hmin)
v = euler_simple(v,paso,roce,masa)
IF (ymax<v(2)) THEN
ymax = v(2)
END IF
WRITE(2,'(2F10.4)') v(1), v(2)
...
CLOSE(2)
CALL SYSTEM ("gnuplot -persist script_ES.p")
...
FUNCTION cambiopaso (v,h,tolerancia,roce,masa,metodo,hmax,hmin)
    ...
    h_encontrado = .FALSE.
    DO WHILE (.NOT. h_encontrado)
    
    IF (metodo==1) THEN  
    a = euler_simple(v, h, roce, masa)
    b = euler_simple(v, h/2, roce, masa)
    c = euler_simple(b, h/2, roce, masa)
